...
def run_batch(config: dict, dry_run: bool = False) -> None:
	...
	prompts = [
		p for d in difficulties for p in iter_prompts(config["prompts_path"], difficulty=d)
	]
	...
	for model in models:
		for prompt in prompts:
			for trial in range(1, trials + 1):
				...
				initial_validation_path = f"{dirs['validation']}/{run_key}_{timestamp}.json"
	...
	section_print(f"Batch complete — {planned} (model, prompt, trial) runs. Results in {results_csv}")


if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Run the full model x prompt x trial x condition experiment sweep.")
	parser.add_argument(
